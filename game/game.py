...
from ai_enhancement import AIGameIntegration  # Thêm import AI
...
main_font = pygame.font.Font("fonts/Roboto-Regular.ttf", 16)
guide_font = pygame.font.Font("fonts/Roboto-Regular.ttf", 15)
title_font = pygame.font.Font("fonts/Roboto-Regular.ttf", 40)
...
def draw_status(surface):
    ...
    block_size = 25
    piece_width = len(next_piece[0]) * block_size
    piece_height = len(next_piece) * block_size
...
def detect_hand_gesture_enhanced(frame, current_piece, grid, game_state):
    ...
    enhanced_gesture = ai_integration.process_gesture_with_ai(
        gesture, current_piece, grid, game_state
    )

    cv2.imshow('Hand Tracking', frame)
    return enhanced_gesture
...
ai_integration = AIGameIntegration()
ai_integration.load_ai_data()
...
while running:
    ...
    if not paused and not game_over:
        ...
        game_state = 'normal'
        if lines_cleared >= 10:
            game_state = 'combo' if lines_cleared % 10 >= 4 else 'urgent'

        now = time.time()
        gesture = detect_hand_gesture_enhanced(frame, current_piece, grid, game_state)
        ai_hint_now = gesture.get('ai_hint', None)
        if now - gesture_timer > gesture_delay:
            ...
            if 'ai_hint' in gesture:
                hint_text = main_font.render(f"Gợi ý AI: {gesture['ai_hint']}", True, (255, 255, 0))
                screen.blit(hint_text, (cols * cell_size + 20, rows * cell_size - 30))
    ...
    if ai_hint_now:
        hint_text = main_font.render(f"Gợi ý AI: {ai_hint_now}", True, (255, 255, 0))
        screen.blit(hint_text, (cols * cell_size + 20, rows * cell_size - 30))
